...
const AdminDashboard: React.FC = () => {
  ...
  const renderTeam = () => (
    <div className="space-y-8 animate-in fade-in slide-in-from-right-8 duration-500 text-left">
      <div className="flex justify-between items-center bg-white dark:bg-app-card p-6 rounded-[2rem] border border-slate-200 dark:border-app-border shadow-sm dark:shadow-xl">
        <div>
          <h2 className="text-2xl font-black text-slate-900 dark:text-white uppercase tracking-tighter text-left">Команда</h2>
          <p className="text-[10px] text-slate-500 dark:text-app-muted font-bold uppercase tracking-widest text-blue-500">Доступы и полномочия</p>
        </div>
  ...
  const renderSecurity = () => (
    <div className="space-y-8 animate-in fade-in slide-in-from-right-8 duration-500 text-left">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="text-3xl font-black text-slate-900 dark:text-white uppercase tracking-tighter text-red-500">Безопасность</h2>
          <p className="text-[10px] font-black text-slate-400 dark:text-app-muted uppercase tracking-[0.4em]">Журнал событий и угроз</p>
        </div>
      </div>
      <div className="bg-white dark:bg-app-card rounded-[3rem] border border-slate-200 dark:border-app-border overflow-hidden shadow-sm dark:shadow-2xl">
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 dark:bg-app-bg text-blue-500 font-black gap-4 animate-pulse">
      ...
      <aside className="w-24 md:w-80 bg-white dark:bg-[#111827] border-r border-slate-200 dark:border-white/10 flex flex-col p-6 space-y-10 z-50 shadow-2xl">
        <div className="flex items-center gap-4 px-2">
          ...
          <div className="hidden md:block">
            ...
            <p className="text-[9px] font-black text-blue-500 uppercase tracking-widest mt-1 text-left">Центр управления</p>
          </div>
        </div>
        ...
        <nav className="flex-1 space-y-3">
          ...
          <button 
            ...
            className="w-full flex items-center gap-5 p-5 rounded-[1.25rem] transition-all text-slate-500 dark:text-app-muted hover:bg-slate-100 dark:hover:bg-white/5 hover:text-slate-900 dark:hover:text-white group"
          >
        ...
        <Modal onClose={() => setIsModalOpen(false)} title="Новый аккаунт">
          <form onSubmit={handleAddUser} className="space-y-6">
            ...
            <div className="space-y-1 text-left">
              <label className="text-[10px] font-black text-slate-400 dark:text-app-muted uppercase ml-2 tracking-widest">Уровень доступа</label>
              <select className="w-full bg-slate-50 dark:bg-app-bg border border-slate-200 dark:border-app-border p-5 rounded-2xl outline-none focus:ring-4 focus:ring-blue-500/10 dark:text-white font-black text-xs uppercase appearance-none cursor-pointer" onChange={e => setNewUser({...newUser, role: e.target.value})}>
...
const NavItem = ({ active, icon, label, onClick }: any) => (
  <button 
    onClick={onClick} 
    className={`w-full flex items-center gap-5 p-5 rounded-[1.25rem] transition-all relative group ${active ? 'bg-blue-600 text-white shadow-[0_15px_30px_rgba(37,99,235,0.3)] scale-[1.02]' : 'text-slate-500 dark:text-app-muted hover:bg-slate-100 dark:hover:bg-white/5 hover:text-slate-900 dark:hover:text-white'}`}
  >
...
const StatCard = ({ label, value, suffix, icon, color }: any) => {
  ...
  return (
    <div className="bg-white dark:bg-app-card p-10 rounded-[3rem] border border-slate-200 dark:border-app-border shadow-sm dark:shadow-2xl dark:shadow-black/5 flex items-center justify-between group hover:scale-[1.03] transition-all relative overflow-hidden text-left">
      <div className="relative z-10 text-left">
        <p className="text-[10px] font-black text-slate-400 dark:text-app-muted uppercase tracking-[0.4em] mb-4 text-left">{label}</p>
        <div className="flex items-baseline gap-2 text-left">
          <span className="text-4xl font-black text-slate-900 dark:text-white font-mono tracking-tighter leading-none">{value}</span>
          {suffix && <span className="text-xs font-black text-slate-400 dark:text-app-muted uppercase">{suffix}</span>}
        </div>
      </div>
...
const UserCard = ({ user, onDelete }: any) => (
  <div className="bg-white dark:bg-app-card p-8 rounded-[2.5rem] border border-slate-200 dark:border-app-border shadow-sm dark:shadow-xl flex items-center justify-between group hover:border-blue-500/20 transition-all hover:-translate-y-1 text-left">
    <div className="flex items-center gap-5 text-left">
      <div className="w-14 h-14 bg-slate-50 dark:bg-app-bg/50 rounded-2xl flex items-center justify-center font-black text-blue-500 border border-slate-100 dark:border-app-border uppercase text-xl shadow-inner group-hover:scale-105 transition-transform">
        {user.username[0]}
      </div>
      <div className="text-left">
        <p className="font-black text-slate-900 dark:text-white uppercase text-base tracking-tight text-left block">{user.username}</p>
        <p className="text-[10px] font-black text-slate-400 dark:text-app-muted uppercase tracking-[0.2em] text-left block">{user.role === 'admin' ? 'Администратор' : user.role === 'cashier' ? 'Кассир' : 'Финансист'}</p>
      </div>
    </div>
    <button onClick={onDelete} className="p-3 text-slate-300 dark:text-gray-500 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-xl transition-all opacity-0 group-hover:opacity-100">
...
const SecurityTable = ({ logs, compact }: any) => (
  <div className="overflow-x-auto text-left">
    <table className="w-full text-left font-mono text-[13px]">
      <thead className={`text-slate-400 dark:text-app-muted uppercase font-black tracking-widest border-b border-slate-100 dark:border-app-border ${compact ? 'text-[10px]' : 'text-[11px]'}`}>
        <tr>
      ...
      <tbody className="divide-y divide-slate-50 dark:divide-app-border">
        {logs?.map((log: any) => (
          <tr key={log.id} className="hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
            ...
            <td className="p-6 text-slate-600 dark:text-gray-300 font-medium">{log.description}</td>
            <td className="p-6 text-right text-slate-400 dark:text-gray-500 text-xs">{new Date(log.created_at).toLocaleString()}</td>
          </tr>
        ))}
      </tbody>
    </table>
...
const Modal = ({ title, children, onClose }: any) => (
  <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[100] flex items-center justify-center p-6">
    <div className="bg-white dark:bg-app-card rounded-[3rem] p-12 max-w-md w-full shadow-[0_0_80px_rgba(0,0,0,0.2)] dark:shadow-[0_0_80px_rgba(0,0,0,0.4)] animate-in fade-in zoom-in-95 relative border border-slate-200 dark:border-app-border text-left">
      <div className="flex justify-between items-center mb-10 text-left">
        ...
        <button onClick={onClose} className="p-3 hover:bg-slate-100 dark:hover:bg-white/10 rounded-full transition-colors"><X className="w-6 h-6" /></button>
      </div>
      {children}
    </div>
  </div>
...
const Input = ({ label, ...props }: any) => (
  <div className="space-y-1 text-left">
    <label className="text-[10px] font-black text-slate-400 dark:text-app-muted uppercase ml-2 tracking-widest text-left block">{label}</label>
    <input {...props} className="w-full bg-slate-50 dark:bg-app-bg border border-slate-200 dark:border-app-border p-5 rounded-2xl outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 dark:text-white font-bold transition-all placeholder:text-slate-400 dark:placeholder:text-gray-300" />
  </div>
);
